...
//update this depending on the db setup
   ...
   FROM rooms WHERE (checkin <= '$checkin' AND checkout >='$checkout') OR (checkin >= '$checkin' AND checkin <='$checkout') OR (checkin <= '$checkin' AND checkout >='$checkin') )";
            ...
            public function booknow($checkin, $checkout, $phone,$roomid, $persons, $userId)
            {
                    
                    $sql="SELECT * FROM booking WHERE room_id='$roomid' and check_in <= '$checkin' AND check_out >='$checkout'";
                    $check= mysqli_query($this->db,$sql)  or die(mysqli_connect_errno()."Data herecannot inserted");;
                 
                    if(mysqli_num_rows($check) == 0)
                    {                        
                        $sql2= "INSERT INTO `booking`(`room_id`, `check_in`, `check_out`, `pax`, `phone`, `user_id`) VALUES ($roomid, '$checkin', '$checkout', $persons, $phone, $userId)";
                    ...
                    {
                        $result = "Room Is Not Available for the selected Day/Time";
                    }
