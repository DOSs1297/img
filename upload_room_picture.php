...
if (isset($_FILES['room_picture']) && $_FILES['room_picture']['error'] == 0) {
    $room_id = $_POST['room_id-picture'];
    $target_dir = "uploads/";
    $original_file_name = basename($_FILES["room_picture"]["name"]);
    ...
    // Check file size
    if ($_FILES["room_picture"]["size"] > 5 * 1024 * 1024) {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["room_picture"]["tmp_name"], $target_file)) {
            $room_picture_path = 'uploads/' . $new_file_name;

            // Update the user profile picture in the database
            if ($con->updateRoomPicture($room_id, $room_picture_path)) {
                $response['success'] = true;
                $response['filepath'] = $room_picture_path;
            } else {
